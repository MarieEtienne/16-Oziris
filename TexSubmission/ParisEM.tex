...
The algorithm proposed in this paper requires an  unbiased estimator of the unknown transition densities and we propose to use  generalized Poisson estimators defined in \cite{fearnhead:papaspiliopoulos:roberts:2008}.
 The use of generalized Poisson estimators entails that the diffusion coefficient $\Gamma$ is constant and equals to the identity matrix.
  Note that, when this is not the case, there exists a class of diffusion processes that may be transformed using Lamperti transformation to satisfy this assumption. 
...
For all $0\le k \le n$, the distribution of $Y_k$ given $X_k$, $X_k$ being a short noation for $X_{t_k}$, has a density with respect to a reference measure $\lambda$ on $\mathbb{R}^m$ given by $g(X_k,\cdot)$. For the sake of silmplicity, we will use the classical short notation $g_k(X_k)$ instead of  $g(X_k,\cdot)$. 
...
The most simple choice for $p_{k-1}$ and $\vartheta_k$ is the bootstrap filter proposed by \cite{gordon:salmond:smith:1993} which sets $p_{k-1} = q_{k-1}$ and for all $x\in\mathbb{R}^d$, $\vartheta(x) = 1$.
 In the case of POD processes $q_{k-1}$ is unknown but it can be replaced by any approximation to sample the particles as any choice of $p_{k-1}$ can be made.
  The approximation can be obtained using a discretization scheme such as Euler method or a Poisson based approximation as detailed below.
   A more appealing choice is the fully adapted particle filter which sets for all $x,x'\in\mathbb{R}^d$, $p_{k-1}(x,x') \propto q_{k-1}(x,x')g_k(x')$ and for all $x\in\mathbb{R}^d$, $\vartheta(x) = \int q_{k-1}(x,x')g_k(x')\mu(\rmd x')$. {\color{blue} C'est si clair que $\vartheta$ est calculable pratiquement ?}
