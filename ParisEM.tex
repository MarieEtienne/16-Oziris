...
\forall~0\leq k\leq n,~\exists \hat{\sigma}_+ \text{ such that }\forall (x,y) \in\mathbb{R}^d\times  \mathbb{R}^d,~~\widehat{q}_k(x,y,\zeta_k)\leq \hat{\sigma_+}
\end{assumpt}
...
\ENDIF
\end{algorithmic}
Then, the conditional probability distribution given $\mathcal{G}_{k+1}^N$ of $J_{k}^{i,\ell}$ is $\Lambda_{k}^N(i,\cdot)$. 
\end{lemma}
\begin{proof}
...
\forall~0\leq k\leq n,~\exists \hat{\sigma}_+ \text{ such that }\forall (j,y) \in [[1,N]]\times  \mathbb{R}^d,~~\widehat{q}_k(\xi^j_k,y,\zeta_k)\leq \hat{\sigma}_+
\end{assumpt}
\vspace{-\baselineskip}
\begin{assumpt}\label{assumpt:local}
\forall~0\leq k\leq n,~\exists \hat{\sigma}_+ \text{ such that }\forall j \in [[1,N]],~~\widehat{q}_k(\xi^j_k,\xi^i_{k+1},\zeta_k)\leq \hat{\sigma}_+
\end{assumpt}
It is worth noting that under assumptions \eqref{assumpt:global} or \eqref{assumpt:ysupport}, the linear complexity property of PaRIS algorithm still holds, whereas if only assumption \eqref{assumpt:local} holds, the resulting algorithm has a quadratic complexity.
%Moreover, the AR algorithm still stands if one can define 
%\begin{equation}
