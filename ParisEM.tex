...
\end{equation}
Therefore, to obtain a generalized random version of PaRIS, we only need to be able to sample from weight $\Lambda_k^N(i,\ell)$ in the case of SDE based HMM.\\
We need to define the following assumption:\\
\[
\textbf{A1: }\forall~0\leq k\leq n,~\exists \hat{\sigma}_+ \text{ such that }\forall (x,y) \in\mathbb{R}^d\times  \mathbb{R}^d,~~\widehat{q}_k(x,y,\zeta_k)\leq \hat{\sigma_+}
\]
Then, the following lemma provides the missing step of a Generalized Random PaRIS (GRand Paris)
\begin{lemma}
\label{lem:AR:unbiased}
Let $0\le k\le n-1$ and $1\le i \le N$, and suppose $A_1$ holds may be upper bounded almost surely by a constant $\hat{\sigma}_+$. Let's define the random variable $J_k^i$ as follow:
\begin{algorithmic}
...
\ENDIF
\end{algorithmic}
Then, the conditional probability distribution given $\mathcal{G}_{k+1}^N$ of $J_{k}^{i,\ell}$ is $\Lambda_{k-1}^N(i,\cdot)$. 
\end{lemma}
\begin{proof}
See Appendix \ref{sec:append:proofs}.
\end{proof}
Note that the assumption \textbf{A1} can be relaxed and replaced by one of the two following properties:
\begin{align*}
\textbf{A2: }\forall~0\leq k\leq n,~\exists \hat{\sigma}_+ \text{ such that }\forall (j,y) \in [[1,N]]\times  \mathbb{R}^d,~~\widehat{q}_k(\xi^j_k,y,\zeta_k)\leq \hat{\sigma_+}\\
\textbf{A3: }\forall~0\leq k\leq n,~\exists \hat{\sigma}_+ \text{ such that }\forall (j,y) \in [[1,N]]\times  \mathbb{R}^d,~~\widehat{q}_k(\xi^j_k,y,\zeta_k)\leq \hat{\sigma_+}
\end{align*}


 for all $1\le j\le N$, $\sup_y \hat{q}_k(\xi_k^j,y,\zeta_k)< \hat{\sigma}_+^k$ almost surely is actually sufficient to apply Algorithm \ref{alg:AR:unknownq}. This improvement keeps the linear complexity property of PaRIS algorithm and Algorithm \ref{alg:Ozaki:PaRIS}, and allows to increase significantly the acceptance rate of Algorithm \ref{alg:AR:unknownq}. %Moreover, the AR algorithm still stands if one can define 
%\begin{equation}
