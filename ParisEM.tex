...
\end{equation}
when the transition density of the hidden states is not available explicitly and where $\{h_k\}_{k=1}^n$ are given functions on $\mathbb{R}^d\times \mathbb{R}^d$.  %In this paper, we are interested in a particular class of Sequential Monte Carlo methods to approximate $\bar{H}_{n}$ known as the PaRIS algorithm introduced in \cite{olsson:westerborn:2016}. 
