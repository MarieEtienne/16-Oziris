...
In that case, the function $\rho_{\Delta_k}$ defined in \eqref{eq:rho:func} can be upper bounded either on $(x,y)$ or only on $y$, the resulting GRand PaRIS algorithm has therefore a linear complexity.

In order to evaluate the performance of our algorithm, we compare the estimation of the EM intermediate quantity with the one obtained by the fixed lag method, for different values of the lag (namely, 1,2,5,10,50). The particle approximation of $\mathcal{Q}(\theta,\theta)$ is computed using each algorithm, see Figure~\ref{fig:res:SINE}. This estimation is performed 200 times with $\tilde{N}=2$, and $M=30$ replications for the Monte Carlo approximation of each $q_k$.  Moreover, the E step requires the computation of a quantity such as \eqref{def:addfunc} with $h_k= \log g_k + \log q_k$.  $\log q_k$ is not available explicitly and is approximated using the unbiased estimator proposed in \cite[Appendix B]{olsson:strojby:2011} based on 30 independent Monte Carlo simulations.
The intermediate quantity of the EM algorithm is also estimated                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      with our algorithm 30 times using $N=5000$ particles, the reference value is then computed as the arithmetic mean of these 30 estimations, see Figure \ref{fig:res:SINE}. 

\textcolor{red}{Detailler plus clairement ce que dit la figure 2 ici (donner clairement les estimateurs voire m\^eme les formules pour le biais et la variance relatifs)}
This experiment was reproduced on a hundred different simulated data, in order study the distribution of the empirical absolute relative bias and coefficient of variations for all methods. The results are shown on Figure \ref{fig:mult:SINE}. 
...
In that case, the $\rho_{\Delta_k}$ function defined in \eqref{eq:rho:func} can be upper bounded as a function of $y$ when $x\in \{\xi_k^1,\dots, \xi_k^N\}$, the resulting GRand PaRIS algorithm has therefore a linear complexity. The intermediate quantity of the EM algorithm is evaluated  as for the SINE model, see Figures~\ref{fig:res:LG} and~\ref{fig:mult:LG}.
...
\end{subfigure}
\caption{\textit{Left: }Process $X$ solution to the SDE \eqref{eq:Lamp:SINE} (black) and observations $Y$ satisfying equation \eqref{eq:obs:SINE} (white) at times $t_0=0,\dots,t_{100}=50$. \textit{Right: } Estimation of the EM intermediate quantity $\mathcal{Q}(\theta_{true},\theta_{true})$  using the fixed lag (FL) technique for 5 different lags (namely 1, 2, 5,10 ,50), and the GRand PaRIS algorithm using 200 replicates. The whiskers represent the extent of the 95\% central values. The dot represents the empirical mean over the 200 replicates. The dotted line shows the reference value, computed using the GRand PaRIS algorithm with $N=5000$ particles.}
\label{fig:res:SINE}
\end{figure}
\begin{figure}[p]
\centering
...
\end{subfigure}
\caption{Evolution of the empirical relative bias for each method in the case of the log-growth model. 100 replications of the experience showed on Figure \ref{fig:res:SINE} are made. The empirical abolute relative bias is computed for each one. The whiskers show the 95\% central values and the dot shows the empirical median. All estimations are performed with an equal computation time.}
\label{fig:mult:SINE}
\end{figure}
\begin{figure}[p]
\centering
...
\end{subfigure}
\caption{\textit{Left: }Process $X$ solution to the SDE \eqref{eq:Lamp:LG} (black) and observations $Y$ satisfying equation \eqref{eq:obs:LG} (white) at times $t_0=0,\dots,t_{100}=50$. \textit{Right: } Estimation of the EM intermediate quantity $\mathcal{Q}(\theta_{true},\theta_{true})$  using the fixed lag (FL) technique for 5 different lags (namely 1, 2, 5,10 ,50), and the GRand PaRIS algorithm using 200 replicates. The whiskers represent the extent of the 95\% central values. The dot represents the empirical mean over the 200 replicates. The dotted line shows the reference value, computed using the GRand PaRIS algorithm with $N=5000$ particles.}
\label{fig:res:LG}
\end{figure}
\begin{figure}[p]
\centering
...
\end{subfigure}
\caption{Empirical distribution of the absolute relative bias and the coefficient of variation of each estimation method. 100 replications of the experience showed on figure \ref{fig:res:LG} are made. The whiskers show the 95\% central values and the dot shows the empirical median. All estimations are performed with an equal computation time.}
\label{fig:mult:LG}
\end{figure}
\section{Conclusions}
