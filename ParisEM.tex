...
The algorithm introduces sufficient statistics $\tau^i_k$, approximating $T_k[h](\xi^i_k)$, for $1\le i\le N$ and $0\le k \le n$ starting with $\tau^i_0 = 0$ for all $1\le i\le N$. Choosing $\tilde{N}\ge 1$, at each time step $0\le k \le {n-1}$ these statistics are updated according to the following steps.
\begin{enumerate}[(i)]
...
an extension of the PaRIS algorithm may be developed by replacing $\qk(\xi^{\ell}_{k},\xi_{k+1}^{i})$  by an  estimator $\widehat{q}_k(\xi^{\ell}_{k},\xi_{k+1}^{i};\zeta_k)$, where $\zeta_k$ is a random variable such that:
\[
...
\begin{proposition}
\label{prop:filter}
Assume that H\ref{assum:boundmodel} and H\ref{assum:boundalgo} hold. Then, for all $0\le k\le n$,
there exist $b_k, c_k>0$ such that for all $N\ge 1$, all $\varepsilon > 0$, and all measurable function $h$ on $\mathsf{X}$,
\begin{equation*}
\mathbb{P}\left( \left|\phi_k^N[h] - \phi_k\left[h\right]\right| \geq \varepsilon \right) \leq b_k \mathrm{exp}\left(-c_k N \epsilon^2\right)\eqsp.
\end{equation*}
\end{proposition}

\begin{proof}
\textcolor{red}{On l'ecrit peut-etre... }
The proof is a straightforward extension of \cite{douc:garivier:moulines:olsson:2011}. 
\end{proof}
...
\subsection*{The Log-growth model}
We now focus on a second well known model in population dynamics, the log-growth model, also used as example in \cite{beskos:papaspiliopoulos:roberts:fearnhead:2006} and \cite{olsson:westerborn:2016}, defined by:
\begin{equation}
\rmd Z_t = \kappa Z_t\left(1-\frac{Z_t}{\gamma}\right)\rmd t + \sigma Z_t \rmd W_t,~~Z_0=Z_0. \label{eq:LG:SDE}
\end{equation}
In order to use Exact algorithms of \cite{beskos:papaspiliopoulos:roberts:fearnhead:2006} and GPE used in \cite{fearnhead:papaspiliopoulos:roberts:2008}, we focus on the version of \eqref{eq:LG:SDE} after the Lamperti transform, i.e., we define $X_t=\eta(Z_t)$, with $\eta(z) := -\log (z)/\sigma$, in this case, $(X_t)_{t\geq 0}$ satisfies the following SDE:
\begin{equation}
\rmd X_t = \overbrace{\left( \frac{\sigma}{2} -  \frac{\kappa}{\sigma} + \frac{\kappa}{\gamma\sigma}\exp\left(-\sigma X_t\right)\right)}^{:=\alpha(X_t)}\rmd t +\rmd W_t,~~X_0=X_0.\label{eq:Lamp:LG}
\end{equation}
In this case, the conditions of the EA2 defined in \cite{beskos:papaspiliopoulos:roberts:fearnhead:2006} are satisfied, as:
\begin{equation}
\forall m \in \mathbb{R}, \exists~\mathsf{U}(m)~\vert~\forall x\geq m, \psi(x):=\alpha^2(x)+\alpha'(x) \leq \mathsf{U}_m.
\end{equation}
Moreover, there exists $\mathsf{L}$ such that $\forall x, \psi(x):=\alpha^2(x)+\alpha'(x)\geq \mathsf{L}$. 
Then, GPE estimators could be computed by simulating a the minimum of a OU bridge, and simulating OU bridges conditionnaly to this minimum. 
However, to the best of our knowledge, no general techniques exist to do so. 
We therefore apply the proposed algorithm, but with a GPE estimator computed with Bessel bridges, as proposed by \cite{beskos:papaspiliopoulos:roberts:fearnhead:2006}.\\
The process solution of \eqref{eq:Lamp:LG} is observed regularly at times $t_0=0,\dots,t_{50}=100$ through the observation process $(Y_k)_{k= 0,\dots,50}$:
\begin{equation}
Y_k = X_k + \varepsilon_k,~~\varepsilon \overset{i.i.d}{\sim} \mathcal{N}(0,\sigma^2_{obs})\label{eq:obs:LG}
\end{equation}
In the example presented here, we fixed the set of  parameter $$\theta_{true} =(\kappa=0.1,\sigma=0.1,\gamma=1000,\sigma^2_{obs}=4).$$\\
In order to evaluate the performance of the algorithm presented here, we compare the estimation of the EM intermediate quantity with the one obtained by the fixed lag method, for different values of the lag (namely, 1,2,5,10,50). 
We therefore compute the particle approximation of $\mathcal{Q}(\theta_{true},\theta_{true})$ for a set of observations presented on Figure \ref{fig:obs:LG}.\\ 
This estimation is performed 300 times, using $N=400$ particles, with $\tilde{N}=2$, and 30 replications for the Monte Carlo approximation of each $q(\xi_k,\xi_{k+1})$. 
Moreover, in the computation of the E step, the corresponding $h_k$ function of equation \eqref{def:addfunc} requires the computation of $\log\left(q(\xi_k,\xi_{k+1})\right)$. 
This quantity is approximated independently from other approximations, using the unbiased estimator proposed in \cite{olsson:strojby:2011} (Appendix B). 
Again, 30 Monte Carlo approximations are used to compute each estimation.\\ In order to obtain a reference value, the same estimation is performed 30 times using $N=10000$ particles (using the proposed algorithm), the reference value is then computed as the arithmetic mean of these 30 estimations. 
The resulting estimates are presented on Figure \ref{fig:res:LG}.\\
The results for the fixed lag technique are in conformity with the ones presented in \cite{olsson:strojby:2011} (Figure 1). 
Indeed, for small lags, athe variance of estimates is small, but showing a bias. 
The bias rapidly decreases as the lag increases, together with a  rapidly increasing variance. 
The PaRIS algorithm for diffusion process presented results in an unbiased estimator, as expected, and show a small variance (lower than the variance for the fixed lag when the lag is one, therefore showing the best results on the presented example. 
...
\caption{One realisation of the process $(X_t)_{t\geq 0}$, solution of the SDE \eqref{eq:Lamp:LG} is represented in white. The observations of $Y$ at times $t_0=0,\dots,t_{50}=100$, satisfying equation \eqref{eq:obs:LG},  are represented in black.}
\label{fig:obs:LG}
\end{figure}
\begin{figure}[p]
\centering
\includegraphics[width=0.8\textwidth]{res_Estep_LG}
\caption{Estimation of the EM intermediate quantity $\mathcal{Q}(\theta_{true},\theta_{true})$ using observations of Figure \ref{fig:obs:LG}, using the fixed lag (FL) technique for 5 different lags (namely 1, 2, 5,10 ,50), and the PaRIS algorithm for diffusion process proposed in this work, using 300 replicates. The whiskers represent the extent of the 95\% central values. The dot represents the empirical mean over the 300 replicates. The dotted line shows the reference value, computed using the unbiased PaRIS algorithm with $N=10000$ particles.}
\label{fig:res:LG}
\end{figure}
