...
\widetilde{\omega}_{k+1}^{i} = \frac{M^{-1}\sum_{m=1}^M \widehat{\qk}(\xi_{k}^{I^{i}_{k+1}},\xi^{i}_{k+1};\zeta_k^m)g_{k+1}(\xi^{i}_{k+1})}{\vartheta_{k+1}(\xi^{I^{i}_{k+1}}_{k}) p_{k} (\xi_{k}^{I^{i}_{k+1}},\xi^{i}_{k+1})}\eqsp,
\end{equation}
...
\STATE Choose $I_{k+1}^{i}$ in $\{1,\ldots,N\}$ with probs $\propto \{\widetilde{\omega}_{k}^1\vartheta_{k+1}(\xi_{k}^1),\dots,\widetilde{\omega}_{k}^N\vartheta_{k+1}(\xi_{k}^N)\}$;
...
\STATE Compute $\widetilde{\omega}^{i}_{k+1}$ using equation \eqref{eq:random:weight}.
\FORALL{$\ell \in 1,\dots, \widetilde N$} 
\STATE \textbf{\sc Sampling Step} Sample independently\\ $j\sim \{\omega_{k}^1,\dots,\omega_{k}^N\}$, $z\sim\zeta_k$, $u\sim \mathcal{U}[0,1]$
\IF{ $$u \leq \frac{\widehat{\qk}(\xi_{k}^j,\xi_{k+1}^i,z)}{\hat{\sigma}_+},$$}
\STATE Set $J_k^{i,\ell} = j$.
